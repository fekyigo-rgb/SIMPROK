...
with terminal echo disabled by psql itself. Only after a password is
confirmed set does the asset promote the role to LOGIN with the same frozen
least-privilege attributes. A failed, mismatched, or cancelled password
entry leaves the role without a password, which the asset detects and turns
into an explicit fail-closed error, rolling the whole transaction back so no
