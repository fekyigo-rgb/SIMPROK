...
describe('W-01 permanent BasicPrice writer inventory', () => {
  it('keeps exactly the two approved writers and no other Prisma writer method', () => {
    const matches = collectTs(sourceRoot).flatMap((file) => {
      ...
      return [...text.matchAll(/(?:tx|prisma)\.basicPrice\.(create|update|updateMany|delete|deleteMany|upsert)\s*\(/g)].map(
        (match) => ({ file, method: match[1] }),
      );
    });
    expect(matches.map(({ file, method }) => ({ file: file.replace(/\\/g, '/').split('/src/')[1], method }))).toEqual([
      { file: 'basic-price/basic-price-publication.service.ts', method: 'update' },
      { file: 'reality-intake/price-submission-review.service.ts', method: 'create' },
    ]);
  });
  ...
  it('publication update writes exactly status and verificationStatus', () => {
    ...
    const update = source.match(/basicPrice\.update\s*\(\{[\s\S]*?data:\s*\{([\s\S]*?)\}\s*,?\s*\}\)/);
    expect(update).not.toBeNull();
    const fields = [...update![1].matchAll(/\b(status|verificationStatus)\s*:/g)].map((m) => m[1]);
