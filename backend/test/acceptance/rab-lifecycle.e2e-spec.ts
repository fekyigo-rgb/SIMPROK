...
describe('PR-35 canonical RAB lifecycle (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const accX = await prisma.project.findFirstOrThrow({ where: { code: ACC_X_CODE }, select: { id: true, workspaceId: true, organizationId: true, status: true } });
    ...
    if (accX.status !== 'ACTIVE') {
      throw new Error(`Expected ACC-X seed status ACTIVE, found ${accX.status} — PROJECT_NOT_DRAFT fixture assumption no longer holds`);
    }
    ...
    const assignedMembership = await prisma.workspaceMembership.findUniqueOrThrow({
      where: { accountId_workspaceId: { accountId: assignedAccount.id, workspaceId: workspaceAId } },
    });
    ...
    await prisma.project.createMany({
      data: [
        { id: PROJECT_ZERO, workspaceId: workspaceAId, organizationId: orgAId, code: 'LC-ZERO', name: 'Lifecycle Zero Draft' },
        { id: PROJECT_ACTIVE_BASELINE, workspaceId: workspaceAId, organizationId: orgAId, code: 'LC-BASELINE', name: 'Lifecycle Active Baseline' },
        { id: PROJECT_APPROVED_RAB, workspaceId: workspaceAId, organizationId: orgAId, code: 'LC-APPROVED', name: 'Lifecycle Approved RAB' },
        { id: PROJECT_DUPLICATE, workspaceId: workspaceAId, organizationId: orgAId, code: 'LC-DUPLICATE', name: 'Lifecycle Duplicate Draft' },
        { id: PROJECT_LAWFUL_IMPORT, workspaceId: workspaceAId, organizationId: orgAId, code: 'LC-LAWFUL', name: 'Lifecycle Lawful Import' },
        { id: PROJECT_TOCTOU_BASELINE, workspaceId: workspaceAId, organizationId: orgAId, code: 'LC-TOCTOU-BASELINE', name: 'Lifecycle TOCTOU Baseline' },
        { id: PROJECT_TOCTOU_APPROVED, workspaceId: workspaceAId, organizationId: orgAId, code: 'LC-TOCTOU-APPROVED', name: 'Lifecycle TOCTOU Approved' },
        { id: PROJECT_TOCTOU_DUPLICATE, workspaceId: workspaceAId, organizationId: orgAId, code: 'LC-TOCTOU-DUPLICATE', name: 'Lifecycle TOCTOU Duplicate' },
      ],
    });

    await prisma.projectAssignment.createMany({
      data: [
        PROJECT_ZERO, PROJECT_ACTIVE_BASELINE, PROJECT_APPROVED_RAB, PROJECT_DUPLICATE, PROJECT_LAWFUL_IMPORT,
        PROJECT_TOCTOU_BASELINE, PROJECT_TOCTOU_APPROVED, PROJECT_TOCTOU_DUPLICATE,
      ].map((projectId) => ({
    ...
    await prisma.boqStructure.create({ data: { id: STRUCTURE_BASELINE_A, projectId: PROJECT_ACTIVE_BASELINE, name: 'Baseline BOQ', version: 1, status: 'LOCKED' } });
    await prisma.rabDocument.create({ data: { id: RAB_ACTIVE_BASELINE, projectId: PROJECT_ACTIVE_BASELINE, boqStructureId: STRUCTURE_BASELINE_A, name: 'Baseline RAB', version: 1, totalBaseCost: 0, totalFinalCost: 0, status: 'APPROVED' } });
    await prisma.projectBaseline.create({ data: { id: BASELINE_ACTIVE_ID, projectId: PROJECT_ACTIVE_BASELINE, rabDocumentId: RAB_ACTIVE_BASELINE, versionNumber: 1, status: 'ACTIVE', approvedAt: new Date() } });
    ...
    await prisma.boqStructure.create({ data: { id: STRUCTURE_BASELINE_B, projectId: PROJECT_APPROVED_RAB, name: 'Approved BOQ', version: 1, status: 'LOCKED' } });
    await prisma.rabDocument.create({ data: { id: RAB_APPROVED_ONLY, projectId: PROJECT_APPROVED_RAB, boqStructureId: STRUCTURE_BASELINE_B, name: 'Approved RAB', version: 1, totalBaseCost: 0, totalFinalCost: 0, status: 'APPROVED' } });

    // PROJECT_DUPLICATE: two Working Drafts — ambiguous authority.
    await prisma.boqStructure.createMany({
      data: [
        { id: STRUCTURE_DUPLICATE_1, projectId: PROJECT_DUPLICATE, name: 'Working Draft', version: 1, status: 'DRAFT' },
        { id: STRUCTURE_DUPLICATE_2, projectId: PROJECT_DUPLICATE, name: 'Working Draft', version: 2, status: 'DRAFT' },
      ],
    });

    // PROJECT_LAWFUL_IMPORT: exactly one empty Working Draft — the positive path.
    await prisma.boqStructure.create({ data: { id: DRAFT_LAWFUL_IMPORT, projectId: PROJECT_LAWFUL_IMPORT, name: 'Working Draft', version: 1, status: 'DRAFT' } });

    // TOCTOU approve fixtures start unblocked with one empty Working Draft each.
    await prisma.boqStructure.createMany({
      data: [
        { id: DRAFT_TOCTOU_BASELINE, projectId: PROJECT_TOCTOU_BASELINE, name: 'Working Draft', version: 1, status: 'DRAFT' },
        { id: DRAFT_TOCTOU_APPROVED, projectId: PROJECT_TOCTOU_APPROVED, name: 'Working Draft', version: 1, status: 'DRAFT' },
        { id: DRAFT_TOCTOU_DUPLICATE, projectId: PROJECT_TOCTOU_DUPLICATE, name: 'Working Draft', version: 1, status: 'DRAFT' },
      ],
    });

    // Account with PROJECT_CREATE but explicitly no RAB_DRAFT_EDIT.
    const permCreate = await prisma.permission.findUniqueOrThrow({ where: { code: 'PROJECT_CREATE' } });
    ...
    const createOnlyAccount = await prisma.account.create({
      data: { email: CREATE_ONLY_EMAIL, passwordHash, displayName: 'Create Only Actor', status: 'ACTIVE' },
    });
    ...
    await prisma.user.create({
      data: { workspaceMembershipId: createOnlyMembership.id, workspaceId: workspaceAId, fullName: 'Create Only Actor', status: 'ACTIVE' },
    });
  ...
  beforeEach(async () => {
    await prisma.boqItem.deleteMany({ where: { boqStructureId: DRAFT_LAWFUL_IMPORT } });
  });
  ...
  afterAll(async () => {
    ...
    await prisma.boqItem.deleteMany({ where: { boqStructure: { projectId: { in: createdProjectIds } } } });
    await prisma.projectBaseline.deleteMany({ where: { projectId: { in: createdProjectIds } } });
    await prisma.rabDocument.deleteMany({ where: { projectId: { in: createdProjectIds } } });
    await prisma.boqStructure.deleteMany({ where: { projectId: { in: createdProjectIds } } });
    await prisma.projectAssignment.deleteMany({ where: { projectId: { in: createdProjectIds } } });

    const createOnlyAccount = await prisma.account.findUnique({ where: { email: CREATE_ONLY_EMAIL } });
    if (createOnlyAccount) {
      const memberships = await prisma.workspaceMembership.findMany({ where: { accountId: createOnlyAccount.id } });
  ...
  describe('zero-draft project (lawful new-project state)', () => {
    it('GET returns a truthful empty draft with capability, PUT creates exactly one Working Draft and reuses it on the next save', async () => {
      ...
      expect(getResponse.body.capability).toMatchObject({ canEnterEditableDraftWorkspace: true, canEditDraft: true, reasonCode: null, workingDraftCount: 0 });
      ...
      const finalDraftCount = await prisma.boqStructure.count({ where: { projectId: PROJECT_ZERO, name: 'Working Draft', status: 'DRAFT' } });
  ...
  describe.each([
    ...
    it(`GET returns 200 with capability.canEditDraft=false and reasonCode ${reasonCode}, without mutating anything`, async () => {
      const draftCountBefore = await prisma.boqStructure.count({ where: { projectId: projectId() } });
      const itemCountBefore = await prisma.boqItem.count({ where: { boqStructure: { projectId: projectId() } } });
      ...
      expect(await prisma.boqStructure.count({ where: { projectId: projectId() } })).toBe(draftCountBefore);
      expect(await prisma.boqItem.count({ where: { boqStructure: { projectId: projectId() } } })).toBe(itemCountBefore);
    });
    ...
    it(`import preview fails closed with 409 ${reasonCode} before any file is parsed`, async () => {
      const response = await postFile(`/projects/${projectId()}/boq/import/preview`, assignedToken).field('selectedSheet', 'RAB').expect(409);
      expect(response.body.message).toBe(reasonCode);
    });
    ...
    it(`import approve fails closed with 409 ${reasonCode} before the fingerprint is even checked`, async () => {
      const response = await postFile(`/projects/${projectId()}/boq/import/approve`, assignedToken)
  ...
  describe('import approve TOCTOU safety — project becomes blocked between a valid preview and approve', () => {
    it('re-evaluates lifecycle inside the approve transaction: ACTIVE_BASELINE_EXISTS wins even with a matching fingerprint', async () => {
      const preview = await postFile(`/projects/${PROJECT_TOCTOU_BASELINE}/boq/import/preview`, assignedToken).field('selectedSheet', 'RAB').expect(201);

      const structureId = '36000000-0000-4000-8000-000000000014';
      const rabId = '36000000-0000-4000-8000-000000000015';
      await prisma.boqStructure.create({ data: { id: structureId, projectId: PROJECT_TOCTOU_BASELINE, name: 'Baseline BOQ', version: 1, status: 'LOCKED' } });
      await prisma.rabDocument.create({ data: { id: rabId, projectId: PROJECT_TOCTOU_BASELINE, boqStructureId: structureId, name: 'Baseline RAB', version: 1, totalBaseCost: 0, totalFinalCost: 0, status: 'APPROVED' } });
      await prisma.projectBaseline.create({ data: { projectId: PROJECT_TOCTOU_BASELINE, rabDocumentId: rabId, versionNumber: 1, status: 'ACTIVE', approvedAt: new Date() } });

      const approve = await postFile(`/projects/${PROJECT_TOCTOU_BASELINE}/boq/import/approve`, assignedToken)
        .field('selectedSheet', 'RAB')
        .field('importFingerprint', preview.body.importFingerprint)
        .expect(409);
      expect(approve.body.message).toBe('ACTIVE_BASELINE_EXISTS');
      expect(await prisma.boqItem.count({ where: { boqStructureId: DRAFT_TOCTOU_BASELINE } })).toBe(0);
    });

    it('re-evaluates lifecycle inside the approve transaction: APPROVED_RAB_EXISTS wins even with a matching fingerprint', async () => {
      const preview = await postFile(`/projects/${PROJECT_TOCTOU_APPROVED}/boq/import/preview`, assignedToken).field('selectedSheet', 'RAB').expect(201);

      const structureId = '36000000-0000-4000-8000-000000000016';
      const rabId = '36000000-0000-4000-8000-000000000017';
      await prisma.boqStructure.create({ data: { id: structureId, projectId: PROJECT_TOCTOU_APPROVED, name: 'Approved BOQ', version: 1, status: 'LOCKED' } });
      await prisma.rabDocument.create({ data: { id: rabId, projectId: PROJECT_TOCTOU_APPROVED, boqStructureId: structureId, name: 'Approved RAB', version: 1, totalBaseCost: 0, totalFinalCost: 0, status: 'APPROVED' } });

      const approve = await postFile(`/projects/${PROJECT_TOCTOU_APPROVED}/boq/import/approve`, assignedToken)
        .field('selectedSheet', 'RAB')
        .field('importFingerprint', preview.body.importFingerprint)
        .expect(409);
      expect(approve.body.message).toBe('APPROVED_RAB_EXISTS');
      expect(await prisma.boqItem.count({ where: { boqStructureId: DRAFT_TOCTOU_APPROVED } })).toBe(0);
    });
    ...
    it('re-evaluates lifecycle inside the approve transaction: a second Working Draft appearing before approve still 409s MULTIPLE_WORKING_DRAFTS', async () => {
      const preview = await postFile(`/projects/${PROJECT_TOCTOU_DUPLICATE}/boq/import/preview`, assignedToken).field('selectedSheet', 'RAB').expect(201);

      const secondDraftId = '36000000-0000-4000-8000-000000000018';
      await prisma.boqStructure.create({ data: { id: secondDraftId, projectId: PROJECT_TOCTOU_DUPLICATE, name: 'Working Draft', version: 2, status: 'DRAFT' } });

      const approve = await postFile(`/projects/${PROJECT_TOCTOU_DUPLICATE}/boq/import/approve`, assignedToken)
        .field('selectedSheet', 'RAB')
        .field('importFingerprint', preview.body.importFingerprint)
        .expect(409);
      expect(approve.body.message).toBe('MULTIPLE_WORKING_DRAFTS');
      expect(await prisma.boqItem.count({ where: { boqStructureId: DRAFT_TOCTOU_DUPLICATE } })).toBe(0);
    });
  });
  ...
  describe('lawful positive journey on an assigned, non-baselined project', () => {
    it('GET -> PUT -> import preview -> import approve all succeed and no baseline/approved-RAB/progress-report is created', async () => {
      ...
      const preview = await postFile(`/projects/${PROJECT_LAWFUL_IMPORT}/boq/import/preview`, assignedToken).field('selectedSheet', 'RAB').expect(201);
      expect(preview.body.canApprove).toBe(true);

      const approve = await postFile(`/projects/${PROJECT_LAWFUL_IMPORT}/boq/import/approve`, assignedToken)
        .field('selectedSheet', 'RAB')
        .field('importFingerprint', preview.body.importFingerprint)
        .expect(201);
      expect(approve.body.importedRows).toBe(4);

      expect(await prisma.projectBaseline.count({ where: { projectId: PROJECT_LAWFUL_IMPORT } })).toBe(0);
      expect(await prisma.rabDocument.count({ where: { projectId: PROJECT_LAWFUL_IMPORT, status: 'APPROVED' } })).toBe(0);
      expect(await prisma.progressReport.count({ where: { projectId: PROJECT_LAWFUL_IMPORT } })).toBe(0);
    });
  });
  ...
  describe('permission activation safety', () => {
    it('PROJECT_CREATE creates exactly one empty Working Draft atomically, with zero baseline/approved-RAB/progress-report', async () => {
      ...
      expect(await prisma.boqItem.count({ where: { boqStructureId: drafts[0].id } })).toBe(0);
      expect(await prisma.projectBaseline.count({ where: { projectId: newProjectId } })).toBe(0);
      expect(await prisma.rabDocument.count({ where: { projectId: newProjectId } })).toBe(0);
      expect(await prisma.progressReport.count({ where: { projectId: newProjectId } })).toBe(0);
  ...
  describe('/projects/mine backend-derived lifecycle projection', () => {
    it('reflects blocked and editable projects correctly, and never derives editability from Project.status', async () => {
      ...
      const byId = new Map<string, { rabLifecycle?: { canEnterEditableDraftWorkspace: boolean; reasonCode: string | null; workingDraftCount: number } }>(
        response.body.map((project: { id: string }) => [project.id, project]),
      );

      expect(byId.get(PROJECT_ACTIVE_BASELINE)?.rabLifecycle).toMatchObject({ canEnterEditableDraftWorkspace: false, reasonCode: 'ACTIVE_BASELINE_EXISTS' });
      expect(byId.get(PROJECT_APPROVED_RAB)?.rabLifecycle).toMatchObject({ canEnterEditableDraftWorkspace: false, reasonCode: 'APPROVED_RAB_EXISTS' });
      expect(byId.get(PROJECT_DUPLICATE)?.rabLifecycle).toMatchObject({ canEnterEditableDraftWorkspace: false, reasonCode: 'MULTIPLE_WORKING_DRAFTS' });
      expect(byId.get(PROJECT_ZERO)?.rabLifecycle).toMatchObject({ canEnterEditableDraftWorkspace: true, reasonCode: null });
      expect(byId.get(accXProjectId)?.rabLifecycle).toMatchObject({ canEnterEditableDraftWorkspace: false, reasonCode: 'PROJECT_NOT_DRAFT' });
    });
  });
  ...
  describe('ACC-X canonical result (Section 4 addendum)', () => {
    it('GET returns 200 with the exact projection shape: PROJECT_NOT_DRAFT, zero baseline/approved/draft facts, no write', async () => {
      const structureCountBefore = await prisma.boqStructure.count({ where: { projectId: accXProjectId } });
      const itemCountBefore = await prisma.boqItem.count({ where: { boqStructure: { projectId: accXProjectId } } });
      ...
      expect(await prisma.boqStructure.count({ where: { projectId: accXProjectId } })).toBe(structureCountBefore);
      expect(await prisma.boqItem.count({ where: { boqStructure: { projectId: accXProjectId } } })).toBe(itemCountBefore);
    });
    ...
    it('a corrupt XLSX on a blocked project reports 409 PROJECT_NOT_DRAFT, not a file-validation error', async () => {
      const response = await request(app.getHttpServer())
        ...
        .set('x-workspace-id', workspaceAId)
        .field('selectedSheet', 'RAB')
        .attach('file', Buffer.from('this is not a real xlsx file'), { filename: 'corrupt.xlsx', contentType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' })
        .expect(409);
    ...
    it('a wrong file extension on a blocked project reports 409 PROJECT_NOT_DRAFT, not ONLY_XLSX', async () => {
      const response = await request(app.getHttpServer())
        ...
        .set('x-workspace-id', workspaceAId)
        .field('selectedSheet', 'RAB')
        .attach('file', Buffer.from('plain text'), { filename: 'wrong-extension.txt', contentType: 'text/plain' })
        .expect(409);
      expect(response.body.message).toBe('PROJECT_NOT_DRAFT');
    });
    ...
    it('an incorrect fingerprint on approve for a blocked project reports 409 PROJECT_NOT_DRAFT, not IMPORT_FINGERPRINT_MISMATCH', async () => {
      const response = await postFile(`/projects/${accXProjectId}/boq/import/approve`, assignedToken)
        .field('selectedSheet', 'RAB')
