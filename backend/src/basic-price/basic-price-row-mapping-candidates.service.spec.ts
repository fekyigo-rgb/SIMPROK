...
} from './basic-price-row-mapping-candidates.service';
import { PrismaService } from '../prisma/prisma.service';

describe('normalizeResourceName', () => {
  it('trims, collapses internal whitespace, and lowercases', () => {
    expect(normalizeResourceName('  Semen   Portland  ')).toBe(
      'semen portland',
    );
...
describe('BasicPriceRowMappingCandidatesService', () => {
  ...
  const baseRow = {
    ...
    batch: {
      ...
      uploadedByAccountId: UPLOADER_ID,
    },
  };
  ...
  beforeEach(async () => {
    ...
    service = module.get<BasicPriceRowMappingCandidatesService>(
      BasicPriceRowMappingCandidatesService,
    );
  });

  it('throws NotFound when the row does not belong to the caller workspace', async () => {
    prisma.basicPriceImportRow.findFirst.mockResolvedValue({
      ...baseRow,
      batch: { ...baseRow.batch, workspaceId: 'other-workspace' },
    });
    await expect(
      service.findCandidatesForRow(WORKSPACE_ID, BATCH_ID, ROW_ID, UPLOADER_ID),
  ...
  it('positive: returns the single unambiguous normalized-name candidate, no provenance signal when no equivalence record exists', async () => {
    ...
    const result = await service.findCandidatesForRow(
      WORKSPACE_ID,
      BATCH_ID,
      ROW_ID,
      UPLOADER_ID,
  ...
  it('negative: an ambiguous name returns every candidate, none singled out or preferred (review signal only, no decision made)', async () => {
    ...
    expect(result.candidates.map((c) => c.resourceCatalogId)).toEqual([
      'resource-01',
      'resource-02',
    ]);
  ...
  it('negative: no match returns an empty candidate list, never a fabricated suggestion', async () => {
    prisma.basicPriceImportRow.findFirst.mockResolvedValue({
      ...baseRow,
      rawResourceNameText: 'Resource With No Catalog Match',
    });
    prisma.$queryRaw.mockResolvedValue([]);
  ...
  it('USER-OWNED IMPORT BOUNDARY negative: a same-workspace account that did not upload this batch cannot see its row candidates (404)', async () => {
    prisma.basicPriceImportRow.findFirst.mockResolvedValue(baseRow);
    await expect(
      service.findCandidatesForRow(
        WORKSPACE_ID,
        BATCH_ID,
        ROW_ID,
  ...
  describe('SOURCE_ROW_PROVENANCE (RM-02D1-REMEDIATION-V3.1)', () => {
    ...
    it('positive: an authorized equivalence record plus a matching provenance identity is surfaced as provenanceCandidate, agreeing with normalized-name — no conflict', async () => {
      ...
      prisma.basicPriceSourceEquivalence.findUnique.mockResolvedValue({
        canonicalSourceSha256: 'canonical-sha',
      });
      prisma.resourceSourceIdentity.findFirst.mockResolvedValue({
        resourceCatalogId: 'resource-01',
      });
      prisma.resourceCatalog.findFirst.mockResolvedValue({
        id: 'resource-01',
        code: null,
        name: 'Semen Portland',
        type: 'MATERIAL',
      ...
      expect(result.provenanceCandidate).toEqual({
        resourceCatalogId: 'resource-01',
        code: null,
        name: 'Semen Portland',
        type: 'MATERIAL',
    ...
    it('conflict: provenance points to a different ResourceCatalog than normalized-name matching — both are returned, conflict is true, neither is dropped or silently preferred', async () => {
      ...
      prisma.$queryRaw.mockResolvedValue([
        {
          resourceCatalogId: 'resource-NAME-MATCH',
          code: null,
          name: 'Semen Portland',
          type: 'MATERIAL',
          baseUnit: 'Zak',
      ...
      prisma.resourceCatalog.findFirst.mockResolvedValue({
        ...
        code: 'X',
        name: 'Semen Portland (provenanced)',
        type: 'MATERIAL',
        baseUnit: 'Zak',
      });
      ...
      expect(result.provenanceCandidate?.resourceCatalogId).toBe(
        'resource-PROVENANCE',
      );
